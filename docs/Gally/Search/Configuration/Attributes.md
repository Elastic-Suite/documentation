...
|Analyzer|Standard|There is 3 possible values. Standard, Reference and Standard_edge_ngram. <br>- Standard : uses the default analyzer, which segments text into words while excluding special characters and stopwords. This analyzer is versatile and suitable for most cases.<br>- Reference: This analyzer is used for fields that must match specific values ​​exactly, such as product references or item codes.<br>- Standard Edge Ngram : This analyzer splits text into substrings (ngrams) from the beginning of words. For example, the word "dresses" will be split into "d", "dr", "dre", "dres", etc. This allows results to be suggested from the first letters entered, ideal for auto-complete searches.|
