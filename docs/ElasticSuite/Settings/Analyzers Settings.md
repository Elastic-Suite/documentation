...
These settings allow you to optimize how the search engine processes zeroes in your product references (such as SKUs, UPCs, or manufacturer part numbers). Depending on your specific part number formatting, enabling these options helps customers find products even if they omit or add zeroes during their search.
...
Optimal SKU Matching: For these settings to work flawlessly across both exact and fuzzy searches, it is highly recommended that you also enable the following experimental settings:
- Search Relevance > Spellchecking Configuration > Term Vectors Configuration > Use all tokens from term vectors
- Search Relevance > Spellchecking Configuration > Term Vectors Configuration > Use reference analyzer in term vectors
- Search Relevance > Relevance Configuration > Exact Match Configuration > Use default analyzer in exact matching filter query

## Language stemmer configuration

This setting is available at the store view scope only.

<img width="990" height="312" alt="Image" src="https://github.com/user-attachments/assets/52a15f96-8600-4c46-bcea-2533c0ed3127" />

This setting determines whether the search engine uses the standard stemming algorithm assigned to your store's locale.
...
<img width="1260" height="539" alt="Image" src="https://github.com/user-attachments/assets/24f46528-29fb-4df7-b053-ac10a54fc989" />

Dictionary stemming provides advanced text analysis for languages that are not natively supported out-of-the-box by standard algorithmic stemmers. This feature relies on the Hunspell token filter.
To utilize this feature, you are required to manually upload Hunspell dictionary files (which can be downloaded from the LibreOffice repository or extracted from a LibreOffice extension) to each node of your Elasticsearch or OpenSearch cluster. These files must be placed within the [config_directory]/hunspell directory.
💡 Best Practice for Dictionary Organization: We highly recommend following the Elasticsearch/OpenSearch naming conventions by organizing your files into subdirectories named after your Magento store view locales (for example: [config_directory]/hunspell/en_US, [config_directory]/hunspell/sl_SI, or [config_directory]/hunspell/vi_VN). Each locale folder should contain exactly one .aff file along with one or more .dic files.
...
<img width="1246" height="706" alt="Image" src="https://github.com/user-attachments/assets/371e31df-c855-4860-8053-19c69213f59b" />

This feature is particularly useful for Germanic languages (such as German and Dutch) that frequently use complex compound words. It leverages language-specific hyphenation patterns—similar to those used by word processors to justify text—to automatically split long compound words into individual, searchable subwords.
To ensure accuracy and prevent the engine from generating meaningless fragments or altering the original meaning of the word, this mechanism relies on an "authorized whitelist" (a dictionary) of subwords. It will only split a compound word if the resulting subwords exist in your whitelist.
...
To eliminate this noise, you can apply one of the following finetuning rules to limit the number of subwords generated.
💡 Best Practices:
It is recommended to enable at most one of these settings. They range from the least restrictive (Only longest match) to the most restrictive (No overlapping matches).
⚠️ Warning: If you enable any of these settings, ensure your custom word list does not include compound words. If it does, the engine might match the compound word and refuse to break it down further.
Reference Example: To understand how each setting behaves, consider the German word kaffeetassenwärmer. If you disable all the settings below, and your word list contains kaffee, tassen, warmer, kaffeetassen, and tassenwarmer, the engine will extract all of these words. Here is how the settings change this behavior:
...
<img width="1303" height="498" alt="Image" src="https://github.com/user-attachments/assets/66d850bf-5133-4b4b-8368-3cfa6ee7b05f" />

How this works By default, the search engine removes most non-alphanumerical characters during text analysis. For example, a product named 3" screw is simply indexed as 3 screw. Consequently, a user searching for 3" screws might see results for 3' screws or 3kg because only the number 3 is matched.
Unit Detection solves this by allowing you to map specific symbols or shorthand notations to their full text names. The engine will automatically find and replace these symbols before text analysis, standardizing how quantities and sizes are searched and indexed.
Standard Units: Map " and in. to inches. Both 3" and 3in. will be indexed and searched exactly as 3 inches.
Custom/Specific Units: Map a notation like # to diameter (preceding the quantity). Searching for #8 will be processed exactly as diameter 8.
...
<img width="1230" height="734" alt="Image" src="https://github.com/user-attachments/assets/f992b7a7-be67-4a5f-9d20-6d185731210b" />
